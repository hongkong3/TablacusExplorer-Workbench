...
TC.Count                                       // FV(タブ)数
FV = TC.Item(N)                                 // インデックス(並び順)指定でFV取得
FV = TC.Selected                               // TC内でアクティブなFVを取得
N = TC.SelectedIndex                           // アクティブFVのインデックス番号, 代入でアクティブFV変更
...
TV = FV.TreeView                             // タブ付属のTreeView？

nFI = FV.Items                               // FV内のファイル/フォルダのアイテム(FI)をCOMコレクションで取得
FI = FV.FocusedItem                          // フォーカス中のアイテム(FI)
nFI = FV.SelectedItems                       // 選択アイテムのCOMコレクション
...
FV = FV.Navigate("path", SBSP_NEWBROWSER);   // 指定パスを新規タブ(FV)で開く
...
pt = te.Data.pt;                  // 最後にマウス操作した位置？
Ctrl = te.CtrlFromPoint(pt);      // 指定スクリーン座標のCtrl取得
elm = document.elementFromPoint(px.x-screenLeft, pt.y-screenTop); // 指定位置のHTMLElement取得

opt = GetAddonElement(Addon_Id);  // アドオン毎のオプション設定操作用
  now = opt.getAttribute('prm');    // オプション値取得
  opt.setAttribute('prm') = new;    //  オプション値設定

GetLangId();                      // 言語設定を取得, en, ja,...
GetText("string");                // ローカライズ化された文字列を取得

importScript("*.js");             // スクリプトの読み込み, TE本体からの相対パス指定可
str = ReadTextFile("path");       // テキストの内容を取得, TE本体からの相対パス指定可

api.PathFileExists("path");       // ファイル存在確認
api.PathIsDirectory("path");      // フォルダ存在確認

"name.ext" = GetFileName("path"); // ファイル名のみ取得
"name" = GetBaseName("path");     // 拡張子を除くファイル名のみ取得
"ext" = GetExtensionName("path"); // 拡張子のみ取得

api.CreateDirectory("path");      // フォルダ作成
api.MoveFileEx("src", "dest", MOVEFILE_COPY_ALLOWED); // ファイル/フォルダ移動 (リネーム)
api.CopyFile("src", "dest", true);  // ファイルコピー
api.SHFileOperation(FO_COPY, "src", "dest", FOF_SILENT|FOF_NOCONFIRMATION, fakse); // フォルダコピー
api.DeleteFile("path");           // ファイル削除
~~~~
